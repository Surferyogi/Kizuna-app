...
function SeijinBackground() {
  ...
  useEffect(() => {
    ...
    // ── Colour palettes ───────────────────────────────────────────────────────
    const FURISODE = [
      { base:'#C8102E', hi:'#E84060', shadow:'#8B0A1E', obi:'#F5D020' },  // Crimson
      { base:'#7B2D8B', hi:'#9B4DAB', shadow:'#4A1A55', obi:'#F0E68C' },  // Purple
      { base:'#1A6B3C', hi:'#2E9455', shadow:'#0F3D22', obi:'#FFD700' },  // Forest green
      { base:'#003087', hi:'#1A52B0', shadow:'#001A4D', obi:'#F5C842' },  // Deep navy
      { base:'#C47020', hi:'#E08C38', shadow:'#7A4510', obi:'#FFFFFF' },  // Burnt orange
      { base:'#8B1A4A', hi:'#B03060', shadow:'#550F2C', obi:'#F0E68C' },  // Deep rose
    ];
    ...
    const petals = Array.from({length:60},(_,i)=>({
      x:r()*W, y:r()*H-H*0.2,
      vx:(r()-0.5)*0.6, vy:0.4+r()*0.8,
      size:2+r()*5, rot:r()*Math.PI*2, rotV:(r()-0.5)*0.04,
      alpha:0.4+r()*0.6, col:r()>0.5?'#FFFFFF':'#FFD7E8',
    }));

    // ── Cherry blossom trees (background) ────────────────────────────────────
    const drawTree = (cx, baseY, scale) => {
      // Trunk
      ctx.strokeStyle = '#5C3317'; ctx.lineWidth = 8*scale;
      ctx.lineCap = 'round';
      ctx.beginPath(); ctx.moveTo(cx, baseY); ctx.lineTo(cx, baseY-80*scale);
      ctx.bezierCurveTo(cx, baseY-100*scale, cx+20*scale, baseY-110*scale, cx+15*scale, baseY-130*scale);
      ctx.stroke();
      ctx.beginPath(); ctx.moveTo(cx, baseY-60*scale); ctx.lineTo(cx-25*scale, baseY-110*scale); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(cx, baseY-70*scale); ctx.lineTo(cx+30*scale, baseY-115*scale); ctx.stroke();
      // Blossoms
      const blossomCentres = [
        [cx, baseY-150*scale,55*scale],[cx-30*scale, baseY-130*scale,40*scale],
        [cx+30*scale, baseY-125*scale,40*scale],[cx+15*scale, baseY-165*scale,35*scale],
        [cx-15*scale, baseY-160*scale,38*scale],
      ];
      blossomCentres.forEach(([bx,by,br]) => {
        // Green leaves
        const lg = ctx.createRadialGradient(bx,by,0,bx,by,br*1.1);
        lg.addColorStop(0,'#4CAF50'); lg.addColorStop(1,'#2E7D32');
        ctx.globalAlpha = 0.35; ctx.fillStyle = lg;
        ctx.beginPath(); ctx.arc(bx,by,br*1.1,0,Math.PI*2); ctx.fill();
        ctx.globalAlpha = 1;
        // Pink cloud
        const pg = ctx.createRadialGradient(bx,by,0,bx,by,br);
        pg.addColorStop(0,'rgba(255,200,210,0.95)');
        pg.addColorStop(0.6,'rgba(255,170,190,0.80)');
        pg.addColorStop(1,'rgba(255,150,170,0)');
        ctx.fillStyle = pg;
        ctx.beginPath(); ctx.arc(bx,by,br,0,Math.PI*2); ctx.fill();
        // Individual blossoms
        for(let b=0;b<12;b++){
          const bx2=bx+(r()-0.5)*br*2, by2=by+(r()-0.5)*br*2;
          ctx.fillStyle='#FFB7C5';
          for(let p=0;p<5;p++){
            const pa=(p/5)*Math.PI*2, px2=bx2+Math.cos(pa)*4*scale, py2=by2+Math.sin(pa)*4*scale;
            ctx.beginPath(); ctx.ellipse(px2,py2,3*scale,2*scale,pa,0,Math.PI*2); ctx.fill();
          }
          ctx.fillStyle='#FFD700';
          ctx.beginPath(); ctx.arc(bx2,by2,1.5*scale,0,Math.PI*2); ctx.fill();
        }
      });
    };

    // ── Draw furisode figure (woman) ──────────────────────────────────────────
    const drawWoman = (cx, groundY, pal, flip, phase) => {
      const S = H * 0.0011;    // scale to screen
      const h = 165 * S;       // total figure height
      const headR = 11 * S;
      ...
      ctx.ellipse(cx, groundY, 22*S, 5*S, 0, 0, Math.PI*2); ctx.fill();
      ctx.restore();

      // ── Furisode body ─────────────────────────────────────────────────────
      const bodyTop = headY + headR*2;
      const waist   = groundY - 60*S;
      const hem     = groundY;

      // Main kimono body gradient
      const kg = ctx.createLinearGradient(cx-30*S, bodyTop, cx+30*S, bodyTop);
      kg.addColorStop(0, pal.shadow); kg.addColorStop(0.4, pal.base); kg.addColorStop(1, pal.hi);
      ctx.fillStyle = kg;
      ctx.beginPath();
      ctx.moveTo(cx-18*S, bodyTop+5*S);
      ctx.bezierCurveTo(cx-24*S, waist, cx-32*S, hem-20*S, cx-40*S, hem);
      ctx.lineTo(cx+40*S, hem);
      ctx.bezierCurveTo(cx+32*S, hem-20*S, cx+24*S, waist, cx+18*S, bodyTop+5*S);
      ctx.closePath(); ctx.fill();

      // Kimono pattern (horizontal bands of geometric motifs)
      ctx.save(); ctx.globalAlpha=0.22;
      for(let row=0;row<6;row++){
        const ry = bodyTop + (hem-bodyTop)*(row+0.5)/6;
        const col = PATTERN_COLS[row%PATTERN_COLS.length];
        ctx.strokeStyle=col; ctx.lineWidth=1.2*S;
        for(let px=-3;px<=3;px++){
          const rx = cx+px*11*S;
          ctx.beginPath();
          ctx.moveTo(rx, ry-4*S); ctx.lineTo(rx+4*S, ry); ctx.lineTo(rx, ry+4*S);
          ctx.lineTo(rx-4*S, ry); ctx.closePath(); ctx.stroke();
        }
      }
      ctx.restore();
      ...
      ctx.lineTo(cx-6*S, bodyTop+25*S);
      ctx.lineTo(cx, bodyTop+30*S);
      ctx.lineTo(cx+6*S, bodyTop+25*S);
      ctx.closePath(); ctx.fill();

      // Obi (sash)
      const obiG = ctx.createLinearGradient(cx-22*S, waist-8*S, cx+22*S, waist+12*S);
      obiG.addColorStop(0,'#B8860B'); obiG.addColorStop(0.5,pal.obi); obiG.addColorStop(1,'#8B6914');
      ctx.fillStyle=obiG;
      ctx.fillRect(cx-22*S, waist-8*S, 44*S, 20*S);
      // Obi knot (elaborate musubi at back — visible as side bump)
      ctx.fillStyle=pal.obi;
      ctx.beginPath();
      ctx.ellipse(cx+(flip?-28*S:28*S), waist, 14*S, 18*S, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.strokeStyle='#8B6914'; ctx.lineWidth=1*S;
      ctx.beginPath();
      ctx.ellipse(cx+(flip?-28*S:28*S), waist, 14*S, 18*S, 0, 0, Math.PI*2);
      ctx.stroke();

      // ── Furisode long sleeves (hanging) ──────────────────────────────────
      const sleeveWave = Math.sin(T*1.2 + phase) * 5 * S;
      [-1,1].forEach((side) => {
        const sx = cx + side*20*S;
        const sleeveG = ctx.createLinearGradient(sx, bodyTop, sx+side*40*S, hem-10*S);
        sleeveG.addColorStop(0, pal.hi); sleeveG.addColorStop(1, pal.shadow);
        ctx.fillStyle = sleeveG;
        ctx.beginPath();
        ctx.moveTo(sx, bodyTop+10*S);
        ctx.bezierCurveTo(
          sx+side*25*S, bodyTop+30*S,
          sx+side*38*S+sleeveWave, waist,
          sx+side*35*S+sleeveWave*1.2, hem-15*S
        );
        ctx.bezierCurveTo(
          sx+side*32*S, hem-5*S,
          sx+side*18*S, hem,
          sx, waist+5*S
        );
        ctx.closePath(); ctx.fill();
        ctx.save(); ctx.globalAlpha=0.18;
        ctx.strokeStyle=PATTERN_COLS[1]; ctx.lineWidth=1*S;
        for(let sr=0;sr<4;sr++){
          const srY=bodyTop+30*S+(hem-bodyTop-30*S)*sr/4;
          ctx.beginPath();
          ctx.moveTo(sx, srY);
          ctx.lineTo(sx+side*30*S, srY+20*S*sr*0.2);
          ctx.stroke();
        }
        ctx.restore();
      });
      ...
      // ── Head ─────────────────────────────────────────────────────────────
      // Hair (elaborate kanzashi updo)
      ctx.fillStyle='#1A1A1A';
      ctx.beginPath();
      ctx.ellipse(cx, headY-headR*0.3, headR*0.95, headR*1.1, 0, 0, Math.PI*2);
      ctx.fill();
      // Hair ornaments (kanzashi)
      ctx.fillStyle='#FFD700';
      ctx.beginPath(); ctx.arc(cx-headR*0.5, headY-headR*1.2, 3*S, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle='#FF69B4';
      ctx.beginPath(); ctx.arc(cx+headR*0.3, headY-headR*1.4, 2.5*S, 0, Math.PI*2); ctx.fill();
      ctx.strokeStyle='#FFD700'; ctx.lineWidth=1.5*S;
      ctx.beginPath(); ctx.moveTo(cx-headR*0.5, headY-headR*1.2); ctx.lineTo(cx-headR*0.3, headY-headR*0.4); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(cx+headR*0.3, headY-headR*1.4); ctx.lineTo(cx+headR*0.2, headY-headR*0.4); ctx.stroke();
      // Face
      const faceG = ctx.createRadialGradient(cx, headY, 0, cx, headY, headR);
      faceG.addColorStop(0,'#FDEBD0'); faceG.addColorStop(1,'#F0C8A0');
      ctx.fillStyle=faceG;
      ctx.beginPath(); ctx.arc(cx, headY, headR, 0, Math.PI*2); ctx.fill();
      // Eyes
      ctx.fillStyle='#1A1A1A';
      [-1,1].forEach(s => {
        ctx.beginPath(); ctx.ellipse(cx+s*4*S, headY-1*S, 2.2*S, 1.4*S, 0, 0, Math.PI*2); ctx.fill();
        ctx.strokeStyle='#FF8FA3'; ctx.lineWidth=0.8*S;
        ctx.beginPath(); ctx.arc(cx+s*4*S, headY-3.5*S, 2.5*S, Math.PI*0.15, Math.PI*0.85); ctx.stroke();
      });
      // Lips
      ctx.fillStyle='#C8566A';
      ctx.beginPath(); ctx.ellipse(cx, headY+3.5*S, 2.5*S, 1.3*S, 0, 0, Math.PI*2); ctx.fill();
      ...
      [-1,1].forEach(s=>{
        ctx.beginPath();
        ctx.ellipse(cx+s*8*S, groundY-2*S, 8*S, 3*S, -0.1*s, 0, Math.PI*2); ctx.fill();
      });
    };

    // ── Draw man in hakama ────────────────────────────────────────────────────
    const drawMan = (cx, groundY, suit, phase) => {
      const S = H * 0.0011;
      const h = 175 * S;
      const headR = 12 * S;
      const headY = groundY - h;
      const shoulder = groundY - h * 0.73;
      const waist    = groundY - h * 0.44;
      const hem      = groundY;

      // Shadow
      ctx.save(); ctx.globalAlpha=0.12;
      ctx.fillStyle='#000'; ctx.beginPath();
      ctx.ellipse(cx, groundY, 20*S, 4*S, 0, 0, Math.PI*2); ctx.fill();
      ctx.restore();
      ...
      if (suit) {
        ...
        ctx.beginPath();
        ctx.moveTo(cx-16*S, waist);
        ctx.bezierCurveTo(cx-18*S, waist+30*S, cx-20*S, hem-30*S, cx-14*S, hem);
        ctx.lineTo(cx, hem); ctx.lineTo(cx, waist);
        ctx.closePath(); ctx.fill();
        ctx.beginPath();
        ctx.moveTo(cx, waist); ctx.lineTo(cx, hem);
        ctx.bezierCurveTo(cx+20*S, hem-30*S, cx+18*S, waist+30*S, cx+16*S, waist);
        ctx.closePath(); ctx.fill();
        // Jacket
        const jg = ctx.createLinearGradient(cx-20*S, shoulder, cx+20*S, shoulder);
        jg.addColorStop(0,'#0A1628'); jg.addColorStop(0.5,'#1C2951'); jg.addColorStop(1,'#0A1628');
        ctx.fillStyle=jg;
        ctx.beginPath();
        ctx.moveTo(cx-20*S, shoulder);
        ctx.lineTo(cx-22*S, waist);
        ctx.lineTo(cx-8*S, waist);
        ctx.lineTo(cx, headY+headR*2.5);
        ctx.lineTo(cx+8*S, waist);
        ctx.lineTo(cx+22*S, waist);
        ctx.lineTo(cx+20*S, shoulder);
        ctx.closePath(); ctx.fill();
        // Lapels
        ctx.fillStyle='#1C3A6B';
        ctx.beginPath();
        ctx.moveTo(cx, headY+headR*2.5);
        ctx.lineTo(cx-8*S, waist);
        ctx.lineTo(cx-3*S, waist-5*S);
        ctx.closePath(); ctx.fill();
        ctx.beginPath();
        ctx.moveTo(cx, headY+headR*2.5);
        ctx.lineTo(cx+8*S, waist);
        ...
        ctx.beginPath();
        ctx.moveTo(cx-3*S, headY+headR*2.5);
        ctx.lineTo(cx-4*S, waist-8*S);
        ctx.lineTo(cx+4*S, waist-8*S);
        ctx.lineTo(cx+3*S, headY+headR*2.5);
        ctx.closePath(); ctx.fill();
        // Tie
        ctx.fillStyle='#8B0000';
        ctx.beginPath();
        ctx.moveTo(cx-1.5*S, headY+headR*2.8);
        ctx.lineTo(cx-2.5*S, waist-12*S);
        ctx.lineTo(cx, waist-8*S);
        ctx.lineTo(cx+2.5*S, waist-12*S);
        ctx.lineTo(cx+1.5*S, headY+headR*2.8);
        ctx.closePath(); ctx.fill();
        // Sleeve arms
        const armSway = Math.sin(T*0.8+phase)*3*S;
        [-1,1].forEach(side=>{
          ctx.fillStyle=jg;
          ctx.beginPath();
          ctx.moveTo(cx+side*20*S, shoulder);
          ctx.bezierCurveTo(cx+side*28*S, shoulder+15*S, cx+side*26*S+armSway, waist-5*S, cx+side*22*S+armSway, waist);
          ctx.lineTo(cx+side*14*S+armSway, waist);
          ctx.bezierCurveTo(cx+side*16*S, shoulder+10*S, cx+side*12*S, shoulder+5*S, cx+side*12*S, shoulder);
          ctx.closePath(); ctx.fill();
          // White cuff
          ctx.fillStyle='#F0F0F0';
          ctx.beginPath();
          ctx.ellipse(cx+side*18*S+armSway, waist, 5*S, 3*S, 0, 0, Math.PI*2); ctx.fill();
        });
      } else {
        // ── Hakama ──────────────────────────────────────────────────────────
        // Hakama (wide pleated trousers) — dark navy/black
        const hakG = ctx.createLinearGradient(cx-25*S, waist, cx+25*S, hem);
        hakG.addColorStop(0,'#1A1A2E'); hakG.addColorStop(0.5,'#2A2A4A'); hakG.addColorStop(1,'#1A1A2E');
        ctx.fillStyle=hakG;
        ctx.beginPath();
        ctx.moveTo(cx-22*S, waist);
        ctx.bezierCurveTo(cx-28*S, waist+30*S, cx-25*S, hem-20*S, cx-18*S, hem);
        ctx.lineTo(cx+18*S, hem);
        ctx.bezierCurveTo(cx+25*S, hem-20*S, cx+28*S, waist+30*S, cx+22*S, waist);
        ctx.closePath(); ctx.fill();
        // Hakama pleats
        ctx.strokeStyle='#0A0A1A'; ctx.lineWidth=1*S;
        for(let p=-2;p<=2;p++){
          ctx.beginPath();
          ctx.moveTo(cx+p*7*S, waist);
          ctx.lineTo(cx+p*7*S*1.1, hem);
          ctx.stroke();
        }
        // Kimono top (haori)
        const hg = ctx.createLinearGradient(cx-18*S, shoulder, cx+18*S, waist);
        hg.addColorStop(0,'#1A1A2E'); hg.addColorStop(0.5,'#2C2C50'); hg.addColorStop(1,'#1A1A2E');
        ctx.fillStyle=hg;
        ctx.beginPath();
        ctx.moveTo(cx-18*S, shoulder);
        ctx.lineTo(cx-20*S, waist);
        ctx.lineTo(cx-8*S, waist);
        ctx.lineTo(cx, shoulder+15*S);
        ctx.lineTo(cx+8*S, waist);
        ctx.lineTo(cx+20*S, waist);
        ctx.lineTo(cx+18*S, shoulder);
        ctx.closePath(); ctx.fill();
        // White collar
        ctx.fillStyle='#FAFAFA';
        ctx.beginPath();
        ctx.moveTo(cx-5*S, shoulder+5*S);
        ctx.lineTo(cx-4*S, waist-5*S);
        ctx.lineTo(cx, waist);
        ctx.lineTo(cx+4*S, waist-5*S);
        ctx.lineTo(cx+5*S, shoulder+5*S);
        ctx.lineTo(cx, shoulder+20*S);
        ctx.closePath(); ctx.fill();
        // Knotted hakama cord
        ctx.fillStyle='#FFFFFF';
        ctx.fillRect(cx-15*S, waist-2*S, 30*S, 4*S);
        ctx.beginPath(); ctx.arc(cx, waist, 5*S, 0, Math.PI*2); ctx.fill();
        // Sleeves
        const armSway2 = Math.sin(T*0.8+phase)*3*S;
        [-1,1].forEach(side=>{
          ctx.fillStyle=hg;
          ctx.beginPath();
          ctx.moveTo(cx+side*18*S, shoulder);
          ctx.bezierCurveTo(cx+side*32*S, shoulder+10*S, cx+side*30*S+armSway2, waist, cx+side*20*S+armSway2, waist+5*S);
          ctx.lineTo(cx+side*10*S+armSway2, waist+5*S);
          ctx.bezierCurveTo(cx+side*12*S, shoulder+8*S, cx+side*10*S, shoulder+3*S, cx+side*10*S, shoulder);
          ctx.closePath(); ctx.fill();
        });
      }
      ...
      // ── Head ─────────────────────────────────────────────────────────────
      const faceG = ctx.createRadialGradient(cx, headY, 0, cx, headY, headR);
      faceG.addColorStop(0,'#F5DEB3'); faceG.addColorStop(1,'#DEB887');
      ctx.fillStyle=faceG;
      ctx.beginPath(); ctx.arc(cx, headY, headR, 0, Math.PI*2); ctx.fill();
      // Hair
      ctx.fillStyle='#111111';
      ctx.beginPath();
      ctx.arc(cx, headY-headR*0.3, headR*0.95, Math.PI, Math.PI*2); ctx.fill();
      ctx.fillRect(cx-headR*0.95, headY-headR*0.35, headR*1.9, headR*0.35);
      // Eyes
      ctx.fillStyle='#1A1A1A';
      [-1,1].forEach(s=>{
        ctx.beginPath(); ctx.ellipse(cx+s*4.5*S, headY-1*S, 2.5*S, 1.6*S, 0, 0, Math.PI*2); ctx.fill();
        // Eyebrow
        ctx.strokeStyle='#333'; ctx.lineWidth=1.2*S;
        ctx.beginPath(); ctx.moveTo(cx+s*2.5*S, headY-4.5*S); ctx.lineTo(cx+s*6.5*S, headY-4*S); ctx.stroke();
      });
      ctx.fillStyle='#8B4040';
      ctx.beginPath(); ctx.ellipse(cx, headY+4*S, 2*S, 1*S, 0, 0, Math.PI*2); ctx.fill();

      // Shoes
      ctx.fillStyle=suit?'#1A1A1A':'#1A1A1A';
      [-1,1].forEach(s=>{
        ctx.beginPath();
        ctx.ellipse(cx+s*7*S, groundY-2*S, 9*S, 3.5*S, 0, 0, Math.PI*2); ctx.fill();
      });
    };

    // ── Group arrangement ─────────────────────────────────────────────────────
    const R2 = RNG(77);
    // 5 women, 3 men, arranged in a semi-realistic group
    const figures = [
      // Back row
      { type:'w', x:W*0.15, y:H*0.92, scale:0.88, pal:FURISODE[1], flip:false, phase:0.0 },
      { type:'m', x:W*0.28, y:H*0.91, scale:0.90, suit:true,  phase:0.5 },
      { type:'w', x:W*0.50, y:H*0.90, scale:0.92, pal:FURISODE[3], flip:true,  phase:1.1 },
      { type:'m', x:W*0.72, y:H*0.91, scale:0.90, suit:false, phase:1.7 },
      { type:'w', x:W*0.86, y:H*0.92, scale:0.88, pal:FURISODE[5], flip:false, phase:2.3 },
      // Front row
      { type:'w', x:W*0.34, y:H*0.97, scale:1.00, pal:FURISODE[0], flip:true,  phase:0.8 },
      { type:'m', x:W*0.52, y:H*0.96, scale:1.02, suit:true,  phase:2.1 },
      { type:'w', x:W*0.66, y:H*0.97, scale:1.00, pal:FURISODE[2], flip:false, phase:1.4 },
      { type:'w', x:W*0.82, y:H*0.96, scale:0.98, pal:FURISODE[4], flip:true,  phase:2.9 },
    ];
    ...
    const frame = (now) => {
      ...
      // Sky — soft pale morning light
      const sky = ctx.createLinearGradient(0,0,0,H);
      sky.addColorStop(0,'#B0C8F0');
      sky.addColorStop(0.35,'#D8EAF8');
      sky.addColorStop(0.65,'#F0E8D8');
      sky.addColorStop(1,'#F5ECD0');
      ctx.fillStyle=sky; ctx.fillRect(0,0,W,H);

      // Sun (low morning sun)
      const sg=ctx.createRadialGradient(W*0.85,H*0.15,0,W*0.85,H*0.15,H*0.18);
      sg.addColorStop(0,'rgba(255,230,160,0.95)');
      sg.addColorStop(0.4,'rgba(255,200,100,0.4)');
      sg.addColorStop(1,'rgba(255,180,80,0)');
      ctx.fillStyle=sg; ctx.beginPath(); ctx.arc(W*0.85,H*0.15,H*0.18,0,Math.PI*2); ctx.fill();

      // Cherry blossom trees (background)
      ctx.save(); ctx.globalAlpha=0.7;
      drawTree(W*0.05, H*0.82, 1.1);
      drawTree(W*0.22, H*0.80, 0.9);
      drawTree(W*0.78, H*0.80, 0.95);
      drawTree(W*0.95, H*0.82, 1.0);
      ctx.restore();

      // Ground / stone plaza
      const grd=ctx.createLinearGradient(0,H*0.88,0,H);
      grd.addColorStop(0,'#E8DCC8'); grd.addColorStop(1,'#D4C8B0');
      ctx.fillStyle=grd; ctx.fillRect(0,H*0.88,W,H*0.12);
      // Stone tiles
      ctx.strokeStyle='rgba(0,0,0,0.08)'; ctx.lineWidth=0.8;
      for(let x=0;x<W;x+=50){ ctx.beginPath();ctx.moveTo(x,H*0.88);ctx.lineTo(x,H);ctx.stroke(); }
      for(let y=H*0.88;y<H;y+=25){ ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W,y);ctx.stroke(); }
      ...
      figures.forEach(f => {
        ...
        ctx.translate(-f.x, 0);
        if (f.type==='w') drawWoman(f.x, f.y, f.pal, f.flip, f.phase);
        else drawMan(f.x, f.y, f.suit, f.phase);
        ctx.restore();
      });

      // Falling petals
      petals.forEach(p => {
        p.x += p.vx + Math.sin(T*0.5+p.phase??0)*0.3;
        p.y += p.vy;
        p.rot += p.rotV;
      ...
      petals.forEach(p => { if(!p.phase) p.phase = Math.random()*Math.PI*2; });

      // Title overlay
      ctx.save();
      ctx.globalAlpha=0.88;
      // Soft glow behind text
      const tg=ctx.createLinearGradient(0,H*0.04,0,H*0.16);
      tg.addColorStop(0,'rgba(255,255,255,0)');
      tg.addColorStop(0.4,'rgba(255,240,220,0.6)');
      tg.addColorStop(1,'rgba(255,255,255,0)');
      ctx.fillStyle=tg; ctx.fillRect(0,H*0.04,W,H*0.12);
      ctx.globalAlpha=1;
      ctx.fillStyle='#2C1A0E';
      ctx.font=`bold ${Math.round(H*0.036)}px "Hiragino Mincho ProN","Yu Mincho","Georgia",serif`;
      ctx.textAlign='center';
      ctx.fillText('成人の日', W*0.5, H*0.092);
      ctx.font=`${Math.round(H*0.018)}px "Hiragino Mincho ProN","Georgia",serif`;
      ctx.fillStyle='#5C3D1A';
      ctx.letterSpacing='0.15em';
      ctx.fillText('SEIJIN NO HI · COMING OF AGE DAY', W*0.5, H*0.125);
      ctx.restore();
...
function DailyQuoteScreen({ quoteData, loading, onDismiss, seasonOverride }) {
  ...
  const isKodomo      = label.includes('kodomo') || label.includes('children\'s day') || label.includes('koinobori');
...
// ─── PUBLIC HOLIDAYS ─────────────────────────────────────────────
// Singapore (SG), Japan (JP) and France (FR) national public holidays 2026-2035.
// Verified: MOM.gov.sg (SG 2026), singaporeholiday.com.sg (SG 2027-2028),
// publicholidays.jp, computed Easter. Hari Raya subject to moon sighting +/-1 day.
const PUBLIC_HOLIDAYS = [
  ...
  {date:'2028-01-26',name:'Chinese New Year',country:'SG'},
  {date:'2028-01-27',name:'Chinese New Year Day 2',country:'SG'},
  {date:'2028-02-27',name:'Hari Raya Puasa',country:'SG'},
  ...
  {date:'2028-05-05',name:'Hari Raya Haji',country:'SG'},
  {date:'2028-05-09',name:'Vesak Day',country:'SG'},
  {date:'2028-08-09',name:'National Day',country:'SG'},
  {date:'2028-10-17',name:'Deepavali',country:'SG'},
  ...
  {date:'2030-02-04',name:'Chinese New Year Day 2',country:'SG'},
  {date:'2030-02-05',name:'Chinese New Year (in lieu)',country:'SG'},
  {date:'2030-02-04',name:'Hari Raya Puasa',country:'SG'},
  ...
  {date:'2031-04-14',name:'Hari Raya Haji (in lieu)',country:'SG'},
  ...
  {date:'2031-08-11',name:'National Day (in lieu)',country:'SG'},
  ...
  {date:'2032-05-03',name:'Labour Day (in lieu)',country:'SG'},
  ...
  {date:'2032-12-27',name:'Christmas Day (in lieu)',country:'SG'},
  // 2033
  {date:'2033-01-01',name:"New Year's Day",country:'SG'},
  {date:'2033-01-03',name:"New Year's Day (in lieu)",country:'SG'},
  ...
  {date:'2033-05-02',name:'Labour Day (in lieu)',country:'SG'},
  {date:'2033-05-22',name:'Vesak Day',country:'SG'},
  {date:'2033-05-23',name:'Vesak Day (in lieu)',country:'SG'},
  {date:'2033-08-09',name:'National Day',country:'SG'},
  {date:'2033-10-30',name:'Deepavali',country:'SG'},
  {date:'2033-10-31',name:'Deepavali (in lieu)',country:'SG'},
  {date:'2033-12-25',name:'Christmas Day',country:'SG'},
  {date:'2033-12-26',name:'Christmas Day (in lieu)',country:'SG'},
  // 2034
  {date:'2034-01-01',name:"New Year's Day",country:'SG'},
  {date:'2034-01-02',name:"New Year's Day (in lieu)",country:'SG'},
  ...
  {date:'2034-11-20',name:'Deepavali (in lieu)',country:'SG'},
  ...
  // ── Japan ────────────────────────────────────────────────────────
  // 2026
  {date:'2026-01-01',name:"New Year's Day",country:'JP'},
  {date:'2026-01-12',name:'Coming of Age Day',country:'JP'},
  {date:'2026-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2026-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2026-03-20',name:'Spring Equinox',country:'JP'},
  {date:'2026-04-29',name:'Showa Day',country:'JP'},
  {date:'2026-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2026-05-04',name:'Greenery Day',country:'JP'},
  {date:'2026-05-05',name:"Children's Day",country:'JP'},
  {date:'2026-07-20',name:'Marine Day',country:'JP'},
  {date:'2026-08-11',name:'Mountain Day',country:'JP'},
  {date:'2026-09-21',name:'Respect for the Aged Day',country:'JP'},
  {date:'2026-09-23',name:'Autumnal Equinox',country:'JP'},
  {date:'2026-09-22',name:"Citizens' Holiday",country:'JP'},
  {date:'2026-10-12',name:'Sports Day',country:'JP'},
  {date:'2026-11-03',name:'Culture Day',country:'JP'},
  {date:'2026-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2027
  {date:'2027-01-01',name:"New Year's Day",country:'JP'},
  {date:'2027-01-11',name:'Coming of Age Day',country:'JP'},
  {date:'2027-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2027-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2027-03-21',name:'Spring Equinox',country:'JP'},
  {date:'2027-04-29',name:'Showa Day',country:'JP'},
  {date:'2027-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2027-05-04',name:'Greenery Day',country:'JP'},
  {date:'2027-05-05',name:"Children's Day",country:'JP'},
  {date:'2027-07-19',name:'Marine Day',country:'JP'},
  {date:'2027-08-11',name:'Mountain Day',country:'JP'},
  {date:'2027-09-20',name:'Respect for the Aged Day',country:'JP'},
  {date:'2027-09-23',name:'Autumnal Equinox',country:'JP'},
  {date:'2027-10-11',name:'Sports Day',country:'JP'},
  {date:'2027-11-03',name:'Culture Day',country:'JP'},
  {date:'2027-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2028
  {date:'2028-01-01',name:"New Year's Day",country:'JP'},
  {date:'2028-01-10',name:'Coming of Age Day',country:'JP'},
  {date:'2028-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2028-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2028-03-20',name:'Spring Equinox',country:'JP'},
  {date:'2028-04-29',name:'Showa Day',country:'JP'},
  {date:'2028-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2028-05-04',name:'Greenery Day',country:'JP'},
  {date:'2028-05-05',name:"Children's Day",country:'JP'},
  {date:'2028-07-17',name:'Marine Day',country:'JP'},
  {date:'2028-08-11',name:'Mountain Day',country:'JP'},
  {date:'2028-09-18',name:'Respect for the Aged Day',country:'JP'},
  {date:'2028-09-22',name:'Autumnal Equinox',country:'JP'},
  {date:'2028-10-09',name:'Sports Day',country:'JP'},
  {date:'2028-11-03',name:'Culture Day',country:'JP'},
  {date:'2028-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2029
  {date:'2029-01-01',name:"New Year's Day",country:'JP'},
  {date:'2029-01-08',name:'Coming of Age Day',country:'JP'},
  {date:'2029-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2029-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2029-03-20',name:'Spring Equinox',country:'JP'},
  {date:'2029-04-29',name:'Showa Day',country:'JP'},
  {date:'2029-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2029-05-04',name:'Greenery Day',country:'JP'},
  {date:'2029-05-05',name:"Children's Day",country:'JP'},
  {date:'2029-07-16',name:'Marine Day',country:'JP'},
  {date:'2029-08-11',name:'Mountain Day',country:'JP'},
  {date:'2029-09-17',name:'Respect for the Aged Day',country:'JP'},
  {date:'2029-09-23',name:'Autumnal Equinox',country:'JP'},
  {date:'2029-10-08',name:'Sports Day',country:'JP'},
  {date:'2029-11-03',name:'Culture Day',country:'JP'},
  {date:'2029-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2030
  {date:'2030-01-01',name:"New Year's Day",country:'JP'},
  {date:'2030-01-14',name:'Coming of Age Day',country:'JP'},
  {date:'2030-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2030-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2030-03-20',name:'Spring Equinox',country:'JP'},
  {date:'2030-04-29',name:'Showa Day',country:'JP'},
  {date:'2030-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2030-05-04',name:'Greenery Day',country:'JP'},
  {date:'2030-05-05',name:"Children's Day",country:'JP'},
  {date:'2030-07-15',name:'Marine Day',country:'JP'},
  {date:'2030-08-11',name:'Mountain Day',country:'JP'},
  {date:'2030-09-16',name:'Respect for the Aged Day',country:'JP'},
  {date:'2030-09-23',name:'Autumnal Equinox',country:'JP'},
  {date:'2030-10-14',name:'Sports Day',country:'JP'},
  {date:'2030-11-03',name:'Culture Day',country:'JP'},
  {date:'2030-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2031
  {date:'2031-01-01',name:"New Year's Day",country:'JP'},
  {date:'2031-01-13',name:'Coming of Age Day',country:'JP'},
  {date:'2031-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2031-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2031-03-21',name:'Spring Equinox',country:'JP'},
  {date:'2031-04-29',name:'Showa Day',country:'JP'},
  {date:'2031-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2031-05-04',name:'Greenery Day',country:'JP'},
  {date:'2031-05-05',name:"Children's Day",country:'JP'},
  {date:'2031-07-21',name:'Marine Day',country:'JP'},
  {date:'2031-08-11',name:'Mountain Day',country:'JP'},
  {date:'2031-09-15',name:'Respect for the Aged Day',country:'JP'},
  {date:'2031-09-23',name:'Autumnal Equinox',country:'JP'},
  {date:'2031-10-13',name:'Sports Day',country:'JP'},
  {date:'2031-11-03',name:'Culture Day',country:'JP'},
  {date:'2031-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2032
  {date:'2032-01-01',name:"New Year's Day",country:'JP'},
  {date:'2032-01-12',name:'Coming of Age Day',country:'JP'},
  {date:'2032-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2032-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2032-03-20',name:'Spring Equinox',country:'JP'},
  {date:'2032-04-29',name:'Showa Day',country:'JP'},
  {date:'2032-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2032-05-04',name:'Greenery Day',country:'JP'},
  {date:'2032-05-05',name:"Children's Day",country:'JP'},
  {date:'2032-07-19',name:'Marine Day',country:'JP'},
  {date:'2032-08-11',name:'Mountain Day',country:'JP'},
  {date:'2032-09-20',name:'Respect for the Aged Day',country:'JP'},
  {date:'2032-09-22',name:'Autumnal Equinox',country:'JP'},
  {date:'2032-10-11',name:'Sports Day',country:'JP'},
  {date:'2032-11-03',name:'Culture Day',country:'JP'},
  {date:'2032-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2033
  {date:'2033-01-01',name:"New Year's Day",country:'JP'},
  {date:'2033-01-10',name:'Coming of Age Day',country:'JP'},
  {date:'2033-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2033-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2033-03-20',name:'Spring Equinox',country:'JP'},
  {date:'2033-04-29',name:'Showa Day',country:'JP'},
  {date:'2033-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2033-05-04',name:'Greenery Day',country:'JP'},
  {date:'2033-05-05',name:"Children's Day",country:'JP'},
  {date:'2033-07-18',name:'Marine Day',country:'JP'},
  {date:'2033-08-11',name:'Mountain Day',country:'JP'},
  {date:'2033-09-19',name:'Respect for the Aged Day',country:'JP'},
  {date:'2033-09-23',name:'Autumnal Equinox',country:'JP'},
  {date:'2033-10-10',name:'Sports Day',country:'JP'},
  {date:'2033-11-03',name:'Culture Day',country:'JP'},
  {date:'2033-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2034
  {date:'2034-01-01',name:"New Year's Day",country:'JP'},
  {date:'2034-01-09',name:'Coming of Age Day',country:'JP'},
  {date:'2034-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2034-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2034-03-21',name:'Spring Equinox',country:'JP'},
  {date:'2034-04-29',name:'Showa Day',country:'JP'},
  {date:'2034-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2034-05-04',name:'Greenery Day',country:'JP'},
  {date:'2034-05-05',name:"Children's Day",country:'JP'},
  {date:'2034-07-17',name:'Marine Day',country:'JP'},
  {date:'2034-08-11',name:'Mountain Day',country:'JP'},
  {date:'2034-09-18',name:'Respect for the Aged Day',country:'JP'},
  {date:'2034-09-23',name:'Autumnal Equinox',country:'JP'},
  {date:'2034-10-09',name:'Sports Day',country:'JP'},
  {date:'2034-11-03',name:'Culture Day',country:'JP'},
  {date:'2034-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // 2035
  {date:'2035-01-01',name:"New Year's Day",country:'JP'},
  {date:'2035-01-08',name:'Coming of Age Day',country:'JP'},
  {date:'2035-02-11',name:'National Foundation Day',country:'JP'},
  {date:'2035-02-23',name:"Emperor's Birthday",country:'JP'},
  {date:'2035-03-20',name:'Spring Equinox',country:'JP'},
  {date:'2035-04-29',name:'Showa Day',country:'JP'},
  {date:'2035-05-03',name:'Constitution Memorial Day',country:'JP'},
  {date:'2035-05-04',name:'Greenery Day',country:'JP'},
  {date:'2035-05-05',name:"Children's Day",country:'JP'},
  {date:'2035-07-16',name:'Marine Day',country:'JP'},
  {date:'2035-08-11',name:'Mountain Day',country:'JP'},
  {date:'2035-09-17',name:'Respect for the Aged Day',country:'JP'},
  {date:'2035-09-23',name:'Autumnal Equinox',country:'JP'},
  {date:'2035-10-08',name:'Sports Day',country:'JP'},
  {date:'2035-11-03',name:'Culture Day',country:'JP'},
  {date:'2035-11-23',name:'Labour Thanksgiving Day',country:'JP'},
  // ── France ────────────────────────────────────────────────────────
  // 2026
...
// Fast lookup by date— Supabase sync across all devices
import { useState, useMemo, useCallback, useEffect, useRef, useContext, createContext } from "react";
import { supabase, supabaseConfigured } from './supabase.js';
import FestiveFireworks, { detectFestiveTheme } from './components/FestiveFireworks.jsx';
...
const OCCASION_DEMOS = [
  { label:"🎏 Kodomo no Hi",  quote:{ quote:"Children do not need to be made to grow — they need space, wonder, and someone steady enough to catch them when they fall. Today we celebrate not what they will become, but everything they already are.", label:"Kodomo no Hi · Children's Day", isSpecial:true } },
  { label:"👘 Seijin no Hi",  quote:{ quote:"Today you cross the threshold — not because you are certain, but because you are ready to become someone who can grow certain. The years ahead will shape you; how you step forward today is already the making of that person.", label:"Seijin no Hi · Coming of Age Day", isSpecial:true } },
